...
const EmailSection: React.FC = () => {
  ...
  const fetchApiKey = async () => {
    // Simulate fetching the API key asynchronously
    return process.env.RESEND_API_KEY; // Replace with your actual fetching logic
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const apiKey = await fetchApiKey(); // Fetch the API key asynchronously

    const response = await fetch("/api/send", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${apiKey}`, // Include the API key in the headers if needed
      },
      body: JSON.stringify(data),
    });
